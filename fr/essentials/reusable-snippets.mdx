...
<Note>
  Toute page dans le`snippets`répertoire sera traitée comme un extrait et ne sera pas
...
1. Ajoutez le contenu que vous souhaitez réutiliser dans votre fichier d'extrait à travers plusieurs
   emplacements. En option, vous pouvez ajouter des variables qui peuvent être remplies via des props
...
<Warning>
  MDX ne se compile pas dans le corps d'une fonction fléchée. Tenez-vous en à la syntaxe HTML
