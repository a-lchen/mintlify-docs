...
每个 Mintlify 站点都需要一个`docs.json`文件来配置核心设置。了解更多关于[属性](#properties)如下。
...
<ResponseField name="colors" type="Colors">
  ...
  <Expandable title="Colors">
    ...
    <ResponseField name="light" type="string">
      深色模式下的主要颜色。在深色模式下最常用于突出显示的内容、章节标题、强调部分
    </ResponseField>
...
<ResponseField name="topbarLinks" type="TopbarLink[]">
  由`name`和`url`组成的数组，包含您想要在顶部栏中包含的链接
...
<ResponseField name="topbarCtaButton" type="Call to Action">
  <Expandable title="Topbar Call to Action">
    <ResponseField name="type" type={'"link" or "github"'} default="link">
      Link 显示一个按钮。GitHub 显示提供的 url 的仓库信息，包括 GitHub 星标数。
    </ResponseField>
    ...
    <ResponseField name="name" type="string">
      按钮内的文本。仅在`type`是`link`时需要。
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="versions" type="string[]">
  版本名称数组。仅在您想要在导航栏中显示不同版本文档的下拉菜单时使用。
</ResponseField>
...
<ResponseField name="anchors" type="Anchor[]">
  ...
  <Expandable title="Anchor">
    <ResponseField name="icon" type="string">
      用于[Font Awesome](https://fontawesome.com/search?q=heart)图标来展示锚点。
    ...
    <ResponseField name="url" type="string">
      标记页面属于该锚点的 URL 开头部分。通常是您放置页面的文件夹名称。
    </ResponseField>
...
<ResponseField name="tabs" type="Tabs[]">
  ...
  <Expandable title="Tabs">
    ...
    <ResponseField name="url" type="string">
      标记页面属于该标签的 URL 开头部分。通常是您放置页面的文件夹名称。
    </ResponseField>
  </Expandable>
</ResponseField>
    ...
    <ResponseField name="auth" type="Auth">
      <Expandable title="Auth">
        ...
        <ResponseField name="inputPrefix" type="string">
          ...
          例如，如果`inputPrefix`是`AuthKey`，身份验证字段的默认输入结果将继承为`AuthKey`。
        </ResponseField>
      </Expandable>
    </ResponseField>
    ...
    <ResponseField name="playground" type="Playground">
      ...
      <Expandable title="Playground">
        <ResponseField name="mode" default="show" type="&#x22;show&#x22; | &#x22;simple&#x22; | &#x22;hide&#x22;">
          playground是显示、隐藏，还是仅显示端点而不添加用户交互功能`simple`
...
<ResponseField name="backgroundImage" type="string">
  显示在每个页面背后的背景图像。参见示例[Infisical](https://infisical.com/docs)和[FRPC](https://frpc.io)。
</ResponseField>
