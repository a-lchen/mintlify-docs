---
title: '全局设置'
description: 'Mintlify通过docs.json文件让您完全控制文档的外观'
icon: 'gear'
---

每个Mintlify站点都需要一个`docs.json`文件，其中包含核心配置设置。在下面了解更多关于[属性](#properties)的信息。
...
<ResponseField name="colors" type="Colors">
  ...
  <Expandable title="Colors">
    <ResponseField name="primary" type="string" required>
      主要颜色。在浅色模式下最常用于突出显示的内容、章节标题和强调
    </ResponseField>

    <ResponseField name="light" type="string">
      深色模式的主要颜色。在深色模式下最常用于突出显示的内容、章节标题和强调
    </ResponseField>
...
<ResponseField name="topbarLinks" type="TopbarLink[]">
  ...
  <Expandable title="TopbarLink">
    ...
    <ResponseField name="url" type="string">
      点击按钮时的URL。示例：`https://mintlify.com/docs`
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="topbarCtaButton" type="Call to Action">
  <Expandable title="Topbar Call to Action">
    <ResponseField name="type" type={'"link" or "github"'} default="link">
      链接显示为按钮。GitHub显示指定URL下的仓库信息，包括GitHub星标数。
    </ResponseField>

    <ResponseField name="url" type="string">
      如果是`link`：按钮链接的目标。

      如果是`github`：要加载GitHub信息的仓库链接。
    </ResponseField>
...
<ResponseField name="versions" type="string[]">
  版本名称数组。仅当您想在导航栏中使用下拉菜单显示不同版本的文档时使用。
</ResponseField>
...
<ResponseField name="anchors" type="Anchor[]">
  ...
  <Expandable title="Anchor">
    <ResponseField name="icon" type="string">
      用于[Font Awesome](https://fontawesome.com/search?q=heart)锚点的图标。
    ...
    <ResponseField name="url" type="string">
      标记属于该锚点的页面的URL开头。通常是存放页面的文件夹名称。
    </ResponseField>

    <ResponseField name="color" type="string">
      锚点图标背景的十六进制颜色。如果传入带有`from`和`to`属性的对象（每个属性都是十六进制颜色），也可以是渐变色。
    </ResponseField>
    ...
    <ResponseField name="isDefaultHidden" type="boolean" default="false">
      传入`true`，如果您想隐藏锚点直到有人直接链接到其中的文档。
    </ResponseField>
...
<ResponseField name="tabs" type="Tabs[]">
  ...
  <Expandable title="Tabs">
    ...
    <ResponseField name="url" type="string">
      标记属于该标签的页面的URL开头。通常是存放页面的文件夹名称。
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="api" type="API">
  API设置配置。在[API组件](/api-playground/demo)中了解更多关于API页面的信息。

  <Expandable title="API">
    <ResponseField name="baseUrl" type="string">
      所有API端点的基础URL。如果`baseUrl`是数组，则允许用户选择多个基础URL选项。
    </ResponseField>
    ...
    <ResponseField name="playground" type="Playground">
      ...
      <Expandable title="Playground">
        <ResponseField name="mode" default="show" type="&#x22;show&#x22; | &#x22;simple&#x22; | &#x22;hide&#x22;">
          是否显示playground，隐藏它，或仅显示端点而不提供额外的用户交互`simple`

          在[playground指南](/api-playground/demo)
        </ResponseField>
      </Expandable>
    </ResponseField>
    ...
    <ResponseField name="maintainOrder" type="boolean">
      ...
      <Warning>此行为很快将成为默认启用，届时此字段将被标记为已弃用。</Warning>
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="footerSocials" type="FooterSocials">
  一个社交媒体账户对象，其中键值对表示社交媒体平台和账户URL。
...
<ResponseField name="modeToggle" type="ModeToggle">
  ...
  <Expandable title="ModeToggle">
    <ResponseField name="default" type={'"light" or "dark"'}>
      设置是否要为新用户始终显示浅色或深色模式。如果未设置，我们默认使用与用户操作系统相同的模式。
    </ResponseField>

    <ResponseField name="isHidden" type="boolean" default="false">
      设置为true以隐藏浅色/深色模式开关。您可以`isHidden`与`default`结合使用，强制您的文档仅使用浅色或深色模式。例如：
...
<ResponseField name="backgroundImage" type="string">
  要在每个页面后显示的背景图像。参见示例[Infisical](https://infisical.com/docs)和[FRPC](https://frpc.io)。
</ResponseField>
